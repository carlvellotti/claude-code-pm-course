...
> **Start this module in Claude Code:** Run `/start-3-1-1` to begin the interactive experience.

## Overview

Module 3.1.1 gets you from zero to your first AI-generated image. You'll set up your Gemini API key, configure billing, and watch as Claude transforms a reference photo into something magical.

**Key takeaway:** The setup is a one-time process. Once your API key is configured, you can generate images with simple natural language requests - Claude handles all the technical complexity.
...
1. In Google AI Studio, go to **Settings** (bottom of left sidebar)
2. Click **Plan information**
3. Click **"Set up Billing"** next to your project
...
The Nano Banana module uses a `.env` file to store your API key securely.

1. In your `nano-banana` folder, find `.env.example`
...
**In the interactive module**, Claude will help you create this file. Just say "Create my .env file with my API key" and Claude handles the file operations.


## Your First Generation

Once setup is complete, you're ready to generate. In the interactive module, Claude uses a reference photo to create a fun welcome image - demonstrating the model's ability to transform real photos.

**What happens behind the scenes:**
1. You describe what you want
2. Claude calls the `generate()` function from `image_gen.py`
3. The function sends your request to Gemini's API
4. The generated image is saved to the `outputs/` folder
5. Claude tells you where to find it
...
When you generate images through Claude Code, you don't need to worry about:

| You handle | Claude handles |
|------------|----------------|
...
This is the power of combining Gemini with Claude Code. You focus on creative direction; Claude handles implementation.


## The `image_gen.py` Module

All image generation runs through a pre-built Python module called `image_gen.py`. You don't need to write any code - Claude uses it on your behalf.
...
Images are numbered sequentially. Claude will tell you the exact path to each generated image so you can open it in Finder (Mac) or Explorer (Windows).
...
- If errors persist, try starting a new session: tell Claude "Start a new session"

### Can't find generated images

- Images save to the `outputs/` folder
- Claude will provide the exact path after each generation
...
Created by [Carl Vellotti](https://www.linkedin.com/in/carlvellotti/). Check out [The Full Stack PM](https://fullstack-pm.com/subscribe?utm_source=ccforpms&utm_medium=course&utm_campaign=nano-banana-welcome) for more PM builder content.
